...
'''
python module to help consume UMB messages
'''
...
class ConfigurationManager(object):
    ...
    def __init__(self, cfg_path=None):
        ...
        with open(cfg_path, 'r', encoding='utf-8') as yaml_data:
    ...
    def get_ca_cert_path(self):
        return self._get_umb_config().get('RH_IT_ROOT_CA')
...
class consumerProcessEvent(object):
    ...
    def process_event(self, event):
        ...
        if isinstance(normalized, dict):
            try:
                ...
                try:
                    ...
                    if str(match[0].value).find(expected_val) != -1:
                        ...
                        self._logger.info("{0}: We are sending processed messages to eventlisteners {1} ".format(threading.currentThread().getName(), self._el))
                        headers = {'content-type': 'application/json'}
                        try:
                            res = requests.post(self._el, data=json.dumps(normalized), headers=headers)
                            self._logger.info(res.content)
                        except (ConnectTimeout, HTTPError, ReadTimeout, Timeout, ConnectionError) as err:
                            self._logger.info("{0}: Ahh, something went wrong! Check sink-url {1} health status".format(threading.currentThread().getName(), self._el))
            ...
            self._logger.info("{0}: We aren't supporting text based messages {1} yet!".format(threading.currentThread().getName(), normalized))
...
class UmbReader(MessagingHandler):
    def __init__(self, topic, selector, sink_url, configmap):
        ...
        self.ca_certs = configmap.get_ca_cert_path()
    ...
    def get_consumer_queue_str(self):
        ...
        return '{}{}{}{}{}'.format(self.consumer, seperator, uuid.uuid1().hex, seperator, self.topic)
    ...
    def on_start(self, event):
        ...
        domain.set_trusted_ca_db(self.ca_certs)
    ...
    def on_connection_closed(self, event):
        self._logger.info("Event: Connection closed")

    def on_connection_closing(self, event):
        self._logger.info("Event: Connection closing")

    def on_connection_error(self, event):
        self._logger.info("Event: Connection error")

    def on_disconnected(self, event):
        self._logger.info("Event: Disconnected")

    def on_link_closed(self, event):
        self._logger.info("Event: Closed")

    def on_link_closing(self, event):
        self._logger.info("Event: Link closing")
    ...
    def on_session_closing(self, event):
        self._logger.info("Event: Session closing")
